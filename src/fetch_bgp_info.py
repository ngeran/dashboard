...
# PASTE format_junos_time FUNCTION HERE
def format_junos_time(uptime_element, peer_state):
    # ... (full content of format_junos_time) ...
...
# PASTE get_bgp_status FUNCTION HERE
def get_bgp_status(hosts, username, password):
    # ... (full content of get_bgp_status) ...
    bgp_data = []
...
if __name__ == "__main__":
    # Get credentials from environment variables for security, or use input()
    JUNOS_HOSTS_STR = os.getenv('JUNOS_HOSTS')
    JUNOS_USERNAME = os.getenv('JUNOS_USERNAME')
    JUNOS_PASSWORD = os.getenv('JUNOS_PASSWORD')

    if not all([JUNOS_HOSTS_STR, JUNOS_USERNAME, JUNOS_PASSWORD]):
        print("Environment variables JUNOS_HOSTS, JUNOS_USERNAME, JUNOS_PASSWORD not set.")
        JUNOS_HOSTS_STR = input("Enter router IP(s) (comma-separated for multiple): ")
        JUNOS_USERNAME = input("Enter username: ")
        JUNOS_PASSWORD = input("Enter password: ")

    JUNOS_HOSTS = [h.strip() for h in JUNOS_HOSTS_STR.split(',')]
